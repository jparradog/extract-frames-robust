...
app = typer.Typer(help="Extrae fotogramas óptimos de videos de broncoscopia priorizando nitidez.")
...
def extract(
    ...
    stage1_dur: float = typer.Option(1.0, help="Duración (s) de segmento de etapa 1 (nitidez)"),
    stage2_dur: float = typer.Option(5.0, help="Duración (s) de segmento de etapa 2 (score combinado)"),
    ...
    w_entropy: float = typer.Option(1.0, help="Peso de entropía en el score de la etapa 2"),
    sharp_percentile: float = typer.Option(0.1, help="Percentil de nitidez para umbral dinámico en etapa 1 (0-1)"),
    top_n: int = typer.Option(1, help="Número de frames top-N por ventana en etapa 1"),
    stage1_stride: float | None = typer.Option(None, help="Stride (s) entre ventanas de etapa 1 (por defecto igual a stage1-dur)")
):
    """
...
def validate(
    ...
    output: str = typer.Option('val_frames', help="Directorio para predicciones temporales"),
    ...
    for video in videos:
        ...
        if not gt_file.exists():
            ...
            continue
        gts = {int(l.strip()) for l in gt_file.read_text().splitlines() if l.strip().isdigit()}
        tp = len(preds & gts)
        total_tp += tp
